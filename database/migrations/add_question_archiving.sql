-- Migration: Add Question Archiving Support
...
CREATE INDEX idx_questions_archived_at ON questions(archived_at);
CREATE INDEX idx_questions_scheduled_deletion ON questions(scheduled_deletion_at);

-- Update the quiz_questions_with_choices view to exclude archived questions
DROP VIEW IF EXISTS quiz_questions_with_choices;
...
-- Update the random quiz function to only use active questions
...
CREATE VIEW admin_questions_view AS
SELECT 
    q.id,
    q.prompt,
    q.explanation,
